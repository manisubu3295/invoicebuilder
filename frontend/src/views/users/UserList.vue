...
async function deletePermanent(user) {
  if (!confirm(`Permanently delete ${user.name}? This cannot be undone.`)) return;
  deletingId.value = user.id;
  try {
    await usersApi.removePermanent(user.id);
      ...
      <table v-else class="mat-table">
        ...
        <tbody>
          ...
          <tr v-for="user in nonDriverUsers" :key="user.id" :class="!user.isActive ? 'opacity-50' : ''">
            ...
            <td class="flex gap-1">
              <button @click="openEdit(user)" class="btn-icon text-gray-400 hover:text-blue-600" title="Edit">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
              </button>
              <button v-if="user.id !== auth.user?.id" @click="deletePermanent(user)" :disabled="deletingId === user.id" class="btn-icon text-gray-400 hover:text-red-600" title="Delete permanently">
